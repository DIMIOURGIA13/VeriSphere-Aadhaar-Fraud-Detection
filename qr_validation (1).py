...
class QRValidator:
    ...
    def _dates_match(self, d1, d2):
        n1 = self._norm(d1).replace('-', '').replace('/', '')
        n2 = self._norm(d2).replace('-', '').replace('/', '')
    ...
    def validate_qr(self, image, ocr_data):
        """
        Detect QR, decode it, compare with OCR data.
        Returns a result dict compatible with the existing web_app.py UI.
        """
        raw_bytes, qr_type = self.detect_qr_code(image)
        ...
        checks = [
            ('aadhaar_number', parsed.get('uid'),  ocr_data.get('aadhaar_number'), 'exact'),
            ('name',           parsed.get('name'), ocr_data.get('name'),           'partial'),
            ('dob',            parsed.get('dob'),  ocr_data.get('dob'),            'date'),
        ]
        ...
        for field, qr_val, ocr_val, mode in checks:
            if qr_val and ocr_val:
                ...
                if match:
                    matches += 1
                details[field] = {'qr_value': qr_val, 'ocr_value': ocr_val, 'match': match}
        ...
        return {
            'qr_found': True,
            'qr_valid': True,
            'qr_format': parsed.get('_format', 'old'),
            'qr_raw_data': raw_bytes.decode('utf-8', errors='replace')[:200],
